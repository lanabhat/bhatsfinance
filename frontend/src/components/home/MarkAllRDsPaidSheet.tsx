...
export function MarkAllRDsPaidSheet({ alerts, accountOptions, onClose, onPaid }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await alertsApi.markAllRDsPaid({
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center" onClick={onClose}>
      <div className="w-full max-w-md rounded-t-2xl bg-[var(--surface)] shadow-xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between border-b border-[var(--border)] px-5 py-4">
          <div>
            <p className="text-base font-semibold text-[var(--text)]">Mark all RD installments as paid</p>
        ...
        <div className="space-y-3 px-5 py-4">
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">Affected installments</p>
