  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Top Holdings</h2>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-100 bg-white">
        {top.map((h, i) => {
          ...
          return (
            <div key={h.instrument_id} className={`px-4 py-3 ${i < top.length - 1 ? 'border-b border-slate-50' : ''}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-900">{h.instrument_name}</p>
                  <p className="text-xs text-slate-400">{h.instrument_type.replace(/_/g, ' ')}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="text-sm font-bold text-slate-900">{fmtINR(h.market_value)}</p>
                  <div className="flex items-center justify-end gap-1.5">
                    <span className="text-xs text-slate-400">{pct.toFixed(1)}% of portfolio</span>
                    {gainPct !== null && (
