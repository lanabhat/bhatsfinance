  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Maturing Soon</h2>
        <span className="text-[10px] text-slate-400">Next {windowDays} days</span>
      </div>
          ...
          return (
            <div key={fd.instrument_id} className={`px-4 py-3 ${isLast ? '' : 'border-b border-amber-100'}`}>
              <div className="flex items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white text-base">🏦</span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-900">{fd.instrument_name}</p>
                ...
                <div className="shrink-0 text-right">
                  ...
                  <p className="text-xs text-slate-500">{formatRemaining(fd.days_remaining)}</p>
                </div>
              </div>
