...
class RDMandate(TimeStampedModel):
    class Frequency(models.TextChoices):
        MONTHLY = 'monthly', 'Monthly'
        QUARTERLY = 'quarterly', 'Quarterly'

    household = models.ForeignKey('core.Household', on_delete=models.CASCADE, related_name='rd_mandates')
    member = models.ForeignKey('core.Member', on_delete=models.SET_NULL, null=True, blank=True, related_name='rd_mandates')
    account = models.ForeignKey('instruments.Account', on_delete=models.CASCADE, related_name='rd_mandates')
    instrument = models.ForeignKey('instruments.Instrument', on_delete=models.CASCADE, related_name='rd_mandates')
...
class RDPaymentAck(TimeStampedModel):
    """Acknowledgement that an RD installment due-date was paid outside the ledger.

    Used when the user marks an installment as paid without deducting cash from a
    tracked account (e.g. paid from an external account, or backfilled from an
    imported statement that already proves the installment was credited).
    Suppresses missed-RD alerts for the corresponding due date.
    """

    mandate = models.ForeignKey(RDMandate, on_delete=models.CASCADE, related_name='payment_acks')
    due_date = models.DateField()
    acknowledged_on = models.DateField()
    note = models.CharField(max_length=200, blank=True)
