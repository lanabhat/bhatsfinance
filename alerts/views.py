...
class RDMandateViewSet(viewsets.ModelViewSet):
    queryset = RDMandate.objects.select_related('household', 'member', 'account', 'instrument').all()
    serializer_class = RDMandateSerializer
    filterset_fields = ['household', 'member', 'account', 'instrument', 'is_active']
    ...
    def mark_all_paid(self, request):
        ...
        for item in items:
            try:
                ...
                continue
            try:
                mandate = RDMandate.objects.get(pk=mandate_id)
            except RDMandate.DoesNotExist:
                results.append({'item': item, 'ok': False, 'error': 'mandate not found.'})
                continue
...
class MissedRDAlertsView(APIView):
    def get(self, request):
        household_id = request.query_params.get('household_id')
        if not household_id:
            return Response({'detail': 'household_id query parameter is required.'}, status=status.HTTP_400_BAD_REQUEST)
