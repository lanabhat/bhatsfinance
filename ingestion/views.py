...
class PpfStatementPreviewView(APIView):
    """Step 1: parse one or more uploaded PPF account statement .xls files."""

    def post(self, request):
        from core.models import Household
        from ingestion.ppf_parser import PpfParseError, parse_ppf_statement
        ...
        for f in files:
            try:
                parsed = parse_ppf_statement(f.read())
            except PpfParseError as e:
                results.append({'filename': f.name, 'error': str(e)})
                continue

            matched_member, confidence = _fuzzy_match_member(parsed.get('holder_name', ''), member_names)
...
class PpfStatementApplyView(APIView):
    """Step 2: commit records from user-confirmed PPF statement parse results."""

    def post(self, request):
        from decimal import Decimal, InvalidOperation

        from core.models import Household, Member
        from ingestion.universal_importer import apply_ppf_statement_import
        ...
        for item in items:
            ...
            estimated_prior_principal = None
            raw_estimate = item.get('estimated_prior_principal')
            if raw_estimate not in (None, ''):
                try:
                    estimated_prior_principal = Decimal(str(raw_estimate))
                except InvalidOperation:
                    pass

            try:
                result = apply_ppf_statement_import(
                    household, member, item,
                    estimated_prior_principal=estimated_prior_principal,
                )
                result['filename'] = item.get('filename', '')
                all_results.append(result)
            except Exception as e:
